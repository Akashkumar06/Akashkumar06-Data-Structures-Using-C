
#include<stdio.h>
#include<stdlib.h>
struct node{
         int info;
         struct node *next;

}*start;
void createList()
{
    struct node *newnode,*curr;
    int item,i,n;

    printf("Enter The size of list:");
    scanf("%d",&n);
    for(i=0;i<n;i++)
          {
            newnode=(struct node*)malloc(sizeof(struct node));
            printf("Enter %d element of list",i+1);
            scanf("%d",&item);

            newnode->info=item;
            newnode->next=NULL;
             if(start==NULL)
                start=newnode;
            else
                curr->next=newnode;
            curr=newnode;
   }
}
void traverse()
{
   struct node *ptr;
   if(start==NULL)
   {
       printf("list is Empty:");
       return;
   }
   printf("linked List Elements are");
   for(ptr=start;ptr!=NULL;ptr=ptr->next)
      printf("%d-->",ptr->info);

}
void insertionAtBeginning()
{
    struct node *newnode;
    int item;
    newnode=(struct node*)malloc(sizeof(struct node));
    if(newnode==NULL)
    {
        printf("Memory is not available:");
         exit(0);
    }

        printf("To insert at beginning ,enter info/data of node:");
        scanf("%d",&item);
        newnode->info=item;
        if(start!=NULL)
             newnode->next=start;
        start=newnode;

    printf("Successfully Node insrted At beginnig!");

}
void insertionAtEnd()
{
    struct node *newnode,*curr;
    int item;
    newnode=(struct node*)malloc(sizeof(struct node));
    if(newnode==NULL)
    {
        printf("Memory is not available:");
         exit(0);
    }
    printf("To insert at beginning ,enter info/data of node:");
        scanf("%d",&item);
     curr=start;
  while(curr->next!=NULL)
      curr=curr->next;
    newnode->info=item;
    newnode->next=NULL;
    curr->next=newnode;
    printf("Successfully Node insrted At End!");
}
void insertionBeforeANode()
{
  struct node *newnode,*curr,*pre;
    int item,nextdata;
    newnode=(struct node*)malloc(sizeof(struct node));
    if(start==NULL)
    {
        printf("memory is not avilable");
    }
    curr=start;
    printf("Enter The item / data before which you wnats to insert the node:");
    scanf("%d",&nextdata);
    printf("Enter  the value you want to be insert");
    scanf("%d",&item);
    while(curr==NULL&&curr->info!=nextdata)
    {   curr=pre;
        curr=curr->next;

    if(curr==NULL)
    {
        printf("The DATA IS NOT PRESENT IN LIST:");

    }
     else
{
        newnode->info=item;
        newnode->next=pre->next;
        pre->next=newnode;
}


    printf("Successfully Node insrted!!!");
}

main()
{
    int ch;
    int value;
    createList();
    do
    {
       printf("\n1.Insertion at Beginning\n2.Insertion at End\n3.insertion before a certain node\n4.Insertion after a cetain node");
       printf("\n5.Deletion at begning\n6.Deletion at End\n7.Deletion after a certain node\n8.deletion befer a certain node\n9.transverse\n10.exit\n");
       printf("Enter your choice:");
       scanf("%d",&ch);
       switch(ch)
       {
           case 1:insertionAtBeginning();
                 break;
           case 2: insertionAtEnd();
                break;
           case 3:insertionBeforeANode();
                break;
           //case 4:insertionAfterANode();
                //  break;

         /*  case 5:deletionAtBegnning();
                 break;
           case 6:deletionAtEnd();
                break;
           case 7:deletionAfterANode();
                     break;
           case 8:deletionBeforeANode();
                    break;*/
           case 9:traverse();
                    break;
           case 10:exit(0);
           default :
                printf("Enter a valid choise:");

       }
    }while(ch!=10);
}
